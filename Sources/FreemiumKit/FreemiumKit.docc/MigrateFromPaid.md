...
>
> I repeat: From now on, never ship a build number that is lower or equal to your `lastPaidRelease` build number. If you do that, you're effectively giving away your app for free. You can easily fix that with a new update that brings the build number back up, but for users who downloaded exactly the version with a build number below (or equal) to your `lastPaidRelease` and never update, it'll be like they had purchased your paid-up-front app. You've been warned!
